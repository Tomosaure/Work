        ...
        for i in 1..Chaine'Last loop
            ...
            for j in 1..3 loop
                if Chaine(i) = Ouvrants(j) Then
                    ...
                    Indice_Erreur := i;
                    Put("Non Correct et Indice_Erreur = ");
                    Put(Indice_Erreur);
                    Correct := False;
                else
                    if Chaine(i) = Fermants(j) and Sommet(pile) = Ouvrants (j) Then
                        Depiler(pile);
                    else
                        Indice_Erreur := i - taille(pile);
                        Put("Non Correct et Indice_Erreur = ");
                        Put(Chaine'Last - Indice_Erreur);
                        Correct := False;
                    end if;
                end if;
            end loop;
