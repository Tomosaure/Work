...
procedure Evaluer_Alea_TH is
	...
	begin
	   Initialiser_TH(Sda);
        Min:=Taille;
        Max:=1;
        --Enregistrement des diffférentes valeurs et leur fréquence
        for i in 1..Taille loop
            Get_Random_Number(Nombre);
            if Cle_Presente_TH(Sda,Nombre) then
                Enregistrer_TH(Sda,Nombre,La_Donnee_TH(Sda,Nombre)+1);
            else
                Enregistrer_TH(Sda,Nombre,1);
            end if;
        end loop;
        --Recherche du min et du max
        for i in 1..Borne loop
            if Cle_Presente_TH(Sda,i) then
                if La_Donnee_TH(Sda,i) < Min then
                    Min := La_Donnee_TH(Sda,i);
                elsif La_Donnee_TH(Sda,i) > Max then
                    Max := La_Donnee_TH(Sda,i);
                end if;
            end if;
        end loop;
...
begin
   ...
   exception
        when others => Put_Line("Erreur de saisie Borne/Taille, Recommencez");
