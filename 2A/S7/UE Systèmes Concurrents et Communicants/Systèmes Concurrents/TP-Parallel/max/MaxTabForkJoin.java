    ...
    static class PartialMax extends RecursiveTask<Integer> {
        ...
        public Integer compute() {
            if (end - start > threshold) {
                PartialMax mp1 = new PartialMax(array, start, (start + end) / 2, threshold);
                PartialMax mp2 = new PartialMax(array, (start + end) / 2, end, threshold);
                mp1.fork();
                mp2.fork();
                return Math.max(mp1.join(), mp2.join());
            } else {
