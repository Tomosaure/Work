...
struct
  ...
  let get_branche = fun e (Noeud(l)) -> List.assoc_opt e l

  let%test _ = get_branche 1 arbre1 = Some (Noeud ([(1,Lettre 'e');(2,Lettre 'b')]))
  let%test _ = get_branche 2 arbre2 = Some (Lettre 'b')
  let%test _ = get_branche 3 arbre2 = Some (Lettre 'c')
  let%test _ = get_branche 5 arbre2 = None
  ...
  let dechiffrer  = fun ls a -> 
    let rec aux l arbre = 
    match l with
    | [] -> if arbre = a then [] else raise CodeNonValide
    | t::q -> match get_branche t arbre with
              | Some (Lettre f) -> f::(aux q a)
              | None -> raise CodeNonValide
              | Some (Noeud(l2)) -> aux q (Noeud(l2))
    in aux ls a
  ...
  let  arbre_to_liste  = fun arbre -> 
    let rec aux arbre l = 
      match arbre with
      | Lettre f -> (f,l)::[]
      | Noeud(l2) -> List.flatten (List.map (fun (e,arbre2) -> aux arbre2 (l@[e])) l2) 
    in aux arbre []
  ...
  let chiffrer   = fun code arbre -> 
    let rec aux l arbre = 
      match l with
      | [] -> []
      | t::q -> match List.assoc_opt t (arbre_to_liste arbre) with
                | None -> raise DonneeNonValide
                | Some l2 -> l2 @ aux q arbre
    in aux code arbre 
...
struct
  ...
  let dechiffrer = fun code arbre -> 
    let code_decompose = DRCode.decompose code in
    let symbole_donnee = Arbre.dechiffrer code_decompose arbre in
    DRDonnee.recompose symbole_donnee
  ...
  let chiffrer = fun donnee arbre -> 
    let decompose_donnee = DRDonnee.decompose donnee in
    let symbole_code = Arbre.chiffrer decompose_donnee arbre in
    DRCode.recompose symbole_code
end
