...
let retract2 p = 
  let rec aux = fun p acc1 acc2 -> 
    match p with
    | [] -> acc1
    | t::q -> if t = 0. 
      then aux q acc1 (t::acc2) 
      else aux q (acc1@acc2@[t]) []
  in aux p [] []

  let retract p = 
    let estVide p = List.for_all (fun x -> x = 0.) p in
    let rec aux p acc = 
      match p with
      | [] -> acc
      | t::q -> if estVide q then 
        if t =0. then acc else (acc@[t])
        else aux q (acc@[t])
    in aux p []
...
let scal_mult = fun x p -> 
  if x = 0. then [] else List.map (fun l -> x*.l) p
...
let plus2  = fun p1 p2 -> 
  let rec aux = fun p1 p2 acc -> 
    ...
    | [],t::q -> aux [] q (acc@[t])
    | t::q,[] -> aux q [] (acc@[t])
    | t1::q1,t2::q2 -> aux q1 q2 (acc@[t1+.t2])
  in aux p1 p2 []
 
  let rec longueur p1 p2 = 
    let (len1, len2) = (List.length p1, List.length p2) in
    if len1 > len2 then longueur p1 (p2@[0.])
    else if len1 < len2 then longueur (p1@[0.]) p2
    else (p1,p2)

    let plus = fun p1 p2 ->
      let (l1, l2) = longueur p1 p2 in
...
*)
let decoder_aux  = fun code a ->
  let rec aux code arbre acc =
  match code , arbre with
  | _, Lettre x -> aux code a (acc@[x])
  | [], _ -> if arbre = a then acc else raise CodeNonValide
  | _, Vide -> raise CodeNonValide
  | 1::q, Noeud (a,_,_) -> aux q a acc
  | 2::q, Noeud (_,b,_) -> aux q b acc
  | 3::q, Noeud (_,_,c) -> aux q c acc
  | _::_, _ -> raise CodeNonValide
  in aux code a []


let decoder = fun code arbre  ->
  let codel = decompose_int code in
  let solution = decoder_aux codel arbre in
  recompose_chaine solution
...
*)
let arbre_to_liste_aux = fun arbre ->
  let rec aux arbre acc = 
  match arbre with
  | Lettre x -> [(x,acc)]
  | Noeud(a,b,c) -> ((aux a (acc@[1])) @ (aux b (acc@[2])) @ (aux c (acc@[3])))
  | Vide -> []
  in aux arbre []

let arbre_to_liste = fun arbre -> 
  let l = arbre_to_liste_aux arbre in 
  List.map (fun (c,x) -> (c, recompose_int x) ) l
...
*)
let encoder = fun mot arbre -> 
  let l = decompose_chaine mot in
  if List.for_all (fun x -> List.mem_assoc x (arbre_to_liste arbre)) l then
    recompose_int (List.map (fun x -> List.assoc x (arbre_to_liste arbre)) l)
  else raise MotNonValide
...
let doublon l = List.fold_left ( fun laux x -> if List.mem x laux then laux else laux@[x]) [] l
