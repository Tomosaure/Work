...
struct
 ...
 let arbre1 =
  ...
  let dechiffrer  = fun code arbre -> 
    let rec aux code arb = 
    match code with 
    | [] -> if arb = arbre then [] else raise CodeNonValide
    | t::q -> match (get_branche t arb) with
              | None -> raise CodeNonValide
              | Some (Lettre x) -> x::(aux q arbre)
  ...
  let rec chiffrer = fun l arbre -> List.fold_left (fun acc t -> match List.assoc_opt t (arbre_to_liste arbre) with 
...
struct
  ...
  let chiffrer = fun code arbre ->
    let decompose_donnee = DRDonnee.decompose code in
    let code_donnee = Arbre.chiffrer decompose_donnee arbre in
    DRCode.recompose code_donnee
