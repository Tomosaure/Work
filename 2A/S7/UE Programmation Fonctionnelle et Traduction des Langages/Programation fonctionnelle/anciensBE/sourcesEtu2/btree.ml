...
let rec height tr = 
    match tr with
    | Leaf _ -> 0
    | Node (trg, trd) -> 1 + max (height trg) (height trd)
...
let rec search tr addrl = 
    ...
    | Leaf x, _ -> x
    | _, [] -> failwith "search"
    | Node(tr1,tr2), t::q -> if t = 0 then search tr1 q else search tr2 q
...
let rec update tr x addrl = 
    match tr, addrl with
        | Leaf _, [] -> Leaf x
        | Leaf _, t::q-> sprout x (t::q)
        | Node(tr1,tr2), t::q -> if t = 0 then Node(update tr1 x q, tr2) else Node(tr1, update tr2 x q)  
        | _, _ -> failwith "update" 
