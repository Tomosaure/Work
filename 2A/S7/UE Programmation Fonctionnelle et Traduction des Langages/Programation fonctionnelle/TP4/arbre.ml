...
let rec parcours arbre =
   let (Noeud (b, lb)) = arbre in
      match lb with
       | [] -> []
       | (tc, ta)::q -> let (Noeud(ba,_)) = ta in if ba then ([tc]::(List.map (fun l -> tc::l) (parcours ta))) @ (parcours (Noeud (b, q))) 
                        else (List.map (fun l -> tc::l) (parcours ta)) @ (parcours (Noeud (b, q)))
       
let%test _ = parcours arbre_sujet = [['b'; 'a'; 's']; ['b'; 'a'; 't']; ['d'; 'e']; ['l'; 'a']; ['l'; 'a'; 'i']; ['l'; 'a'; 'i'; 'd']; ['l'; 'a'; 'i'; 't']; ['l'; 'a'; 'r'; 'd']; ['l'; 'e']; ['l'; 'e'; 's']; ['l'; 'o'; 'n'; 'g']]
