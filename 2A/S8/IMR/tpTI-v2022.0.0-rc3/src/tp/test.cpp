...
{   
    ...
    for (int i = 0; i < data.total(); i++) { 
        ...
        while (!converged) {
            // on calcule la moyenne des pixels voisins de pixel
            Vec3i sum (0, 0, 0);
            int count = 0;
            for (int x = -spatialRad; x <= spatialRad; x++) {
                for (int y = -spatialRad; y <= spatialRad; y++) {
                    // on récupère l'index du pixel voisin
                    ...
                    // on vérifie que le pixel voisin est dans le rayon de couleur
...
int main(int argc, char** argv)
{
    ...
    // on convertit l'image en espace couleur L*a*b :: L : luminosité, a : composante verte-rouge, b : composante bleue-jaune
